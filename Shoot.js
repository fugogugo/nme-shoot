...
ApplicationMain.main = function() {
	...
	ApplicationMain.loaders.set("images/Bullet01.png",loader);
	ApplicationMain.total++;
	var loader1 = new jeash.display.Loader();
	ApplicationMain.loaders.set("images/Enemy01.png",loader1);
	ApplicationMain.total++;
	var loader2 = new jeash.display.Loader();
	ApplicationMain.loaders.set("images/MyShip.png",loader2);
	ApplicationMain.total++;
	if(ApplicationMain.total == 0) ApplicationMain.begin(); else {
		...
		while( $it0.hasNext() ) {
			...
			loader3.load(new jeash.net.URLRequest(path));
		}
...
jeash.display.DisplayObject.prototype = $extend(jeash.events.EventDispatcher.prototype,{
	...
	,__properties__: {set_x:"jeashSetX",get_x:"jeashGetX",set_y:"jeashSetY",get_y:"jeashGetY",set_scaleX:"jeashSetScaleX",get_scaleX:"jeashGetScaleX",set_scaleY:"jeashSetScaleY",get_scaleY:"jeashGetScaleY",set_width:"jeashSetWidth",get_width:"jeashGetWidth",set_height:"jeashSetHeight",get_height:"jeashGetHeight",set_transform:"setTransform",set_visible:"jeashSetVisible",get_visible:"jeashGetVisible",get_mouseX:"jeashGetMouseX",get_mouseY:"jeashGetMouseY",set_parent:"jeashSetParent",get_stage:"getStage",set_filters:"jeashSetFilters",get_filters:"jeashGetFilters",get__boundsInvalid:"getBoundsInvalid",get__matrixChainInvalid:"getMatrixChainInvalid",get__matrixInvalid:"getMatrixInvalid"}
});
...
jeash.display.Sprite.prototype = $extend(jeash.display.DisplayObjectContainer.prototype,{
	...
	,__properties__: $extend(jeash.display.DisplayObjectContainer.prototype.__properties__,{get_graphics:"jeashGetGraphics",set_useHandCursor:"jeashSetUseHandCursor"})
});
...
$hxClasses["Mover"] = Mover;
Mover.__name__ = ["Mover"];
Mover.__super__ = jeash.display.Sprite;
Mover.prototype = $extend(jeash.display.Sprite.prototype,{
	setGraphic: function(path) {
		this.graphic = GraphicCache.loadGraphic(path);
	}
	,setY: function(y) {
		this.jeashSetY(y - this.graphic.jeashGetHeight() / 2.0);
		return this.cy = y;
	}
	,setX: function(x) {
		this.jeashSetX(x - this.graphic.jeashGetWidth() / 2.0);
		return this.cx = x;
	}
	...
	,__class__: Mover
	,__properties__: $extend(jeash.display.Sprite.prototype.__properties__,{set_cx:"setX",set_cy:"setY"})
});
var Bullet = function(x,y) {
	this.setGraphic("images/Bullet01.png");
	Mover.call(this,x,y,this.graphic);
	this.hp = 0;
	this.power = 1;
	this.hitRange = 10.0;
};
...
Bullet.__super__ = Mover;
Bullet.prototype = $extend(Mover.prototype,{
	update: function() {
		this.setY(this.cy - 500.0 / nme.Lib.nmeGetStage().jeashGetFrameRate());
	}
	,__class__: Bullet
});
...
var Enemy = function(initX,initY,graphic) {
	this.score = 0;
...
Enemy.__super__ = Mover;
Enemy.prototype = $extend(Mover.prototype,{
	__class__: Enemy
});
var EnemyFormation = function() {
	this.enemies = new Array();
	this.graphic = new jeash.display.Sprite();
...
EnemyFormation.__name__ = ["EnemyFormation"];
EnemyFormation.__super__ = Enemy;
EnemyFormation.prototype = $extend(Enemy.prototype,{
	...
	,update: function() {
		...
		} else this.jeashSetVisible(false);
		this.frameCount++;
	}
	,__class__: EnemyFormation
});
var Scene = function() {
	jeash.display.Sprite.call(this);
};
...
Scene.__super__ = jeash.display.Sprite;
Scene.prototype = $extend(jeash.display.Sprite.prototype,{
	update: function() {
...
GameScene.prototype = $extend(Scene.prototype,{
	updateTextField: function(textField,text,y) {
		textField.jeashSetY(y);
		textField.SetText(text);
		textField.selectable = false;
		var textFormat = new jeash.text.TextFormat("_sans",20,3355443);
		textFormat.align = jeash.text.TextFormatAlign.LEFT;
		textField.setTextFormat(textFormat);
		textField.alpha = 30;
		com.eclecticdesignstudio.motion.Actuate.tween(textField,1,{ alpha : 1}).delay(0.4);
	}
	...
	,collisionWithEnemyAndBullet: function() {
		var _g = this;
		var compute = function(enemy,enemyFormation,bullet) {
			if(enemy.isHit(bullet)) {
				enemy.hp -= bullet.power;
				_g.removeBullet(bullet);
				if(enemy.hp <= 0) {
					enemy.active = false;
		...
		while(_g1 < _g11.length) {
			...
			while(_g2 < _g3.length) {
				...
				++_g2;
				var _g4 = 0, _g5 = enemyFormation.enemies;
	...
	,detectCollision: function() {
		this.collisionWithEnemyAndBullet();
...
var GraphicCache = function() { }
$hxClasses["GraphicCache"] = GraphicCache;
GraphicCache.__name__ = ["GraphicCache"];
GraphicCache.loadGraphic = function(path,cache) {
	if(cache == null) cache = true;
	var bitmap;
	var bitmapData;
	if(cache) {
		if(!GraphicCache.graphicCache.exists(path)) GraphicCache.graphicCache.set(path,nme.installer.Assets.getBitmapData(path));
		bitmap = new jeash.display.Bitmap(GraphicCache.graphicCache.get(path));
	} else bitmap = new jeash.display.Bitmap(nme.installer.Assets.getBitmapData(path));
	var sprite = new jeash.display.Sprite();
...
var KiteEnemy = function(initX,initY,speedPerSecond) {
	...
	this.random = Std.random(2) == 1;
};
$hxClasses["KiteEnemy"] = KiteEnemy;
KiteEnemy.__name__ = ["KiteEnemy"];
KiteEnemy.__super__ = Enemy;
KiteEnemy.prototype = $extend(Enemy.prototype,{
	update: function() {
		if(this.random) this.setX(this.cx + Math.cos(this.cy / this.speedPerSecond) * 2.0); else this.setX(this.cx - Math.cos(this.cy / this.speedPerSecond) * 2.0);
		this.setY(this.cy + this.speedPerSecond / nme.Lib.nmeGetStage().jeashGetFrameRate());
	}
	,__class__: KiteEnemy
});
var KiteEnemyFormation = function(initX,initY,appearanceSec) {
	EnemyFormation.call(this);
	this.relativeX = initX;
	this.relativeY = initY;
	this.appearanceSec = appearanceSec;
	this.addEnemy(new KiteEnemy(this.relativeX,this.relativeY,Std.random(50) + 100.0));
	this.addEnemy(new KiteEnemy(this.relativeX + 60.0,this.relativeY + 20.0,Std.random(50) + 130.0));
	this.addEnemy(new KiteEnemy(this.relativeX + 120.0,this.relativeY + 10.0,Std.random(50) + 110.0));
};
$hxClasses["KiteEnemyFormation"] = KiteEnemyFormation;
KiteEnemyFormation.__name__ = ["KiteEnemyFormation"];
KiteEnemyFormation.__super__ = EnemyFormation;
KiteEnemyFormation.prototype = $extend(EnemyFormation.prototype,{
	__class__: KiteEnemyFormation
});
var List = function() {
	this.length = 0;
};
...
Main.prototype = $extend(jeash.display.Sprite.prototype,{
	this_onEnterFrame: function(event) {
		var $e = (this.currentScene.update());
		switch( $e[1] ) {
		case 0:
			return;
		case 1:
			var s = $e[2];
			nme.Lib.nmeGetCurrent().removeChild(this.currentScene);
			this.currentScene = s;
			nme.Lib.nmeGetCurrent().addChild(this.currentScene);
			break;
		}
...
var MyShip = function() {
	this.setGraphic("images/MyShip.png");
...
var NextScene = $hxClasses["NextScene"] = { __ename__ : true, __constructs__ : ["Remaining","Next"] }
NextScene.Remaining = ["Remaining",0];
NextScene.Remaining.toString = $estr;
NextScene.Remaining.__enum__ = NextScene;
NextScene.Next = function(s) { var $x = ["Next",1,s]; $x.__enum__ = NextScene; $x.toString = $estr; return $x; }
var Stage1Scene = function() {
	GameScene.call(this);
	this.frameCount = 0;
	this.addEnemyFormation(new KiteEnemyFormation(50.0,0.0,2.0));
	this.addEnemyFormation(new KiteEnemyFormation(200.0,0.0,2.0));
	this.addEnemyFormation(new KiteEnemyFormation(250.0,0.0,5.0));
	this.addEnemyFormation(new KiteEnemyFormation(200.0,0.0,6.0));
	this.addEnemyFormation(new KiteEnemyFormation(150.0,0.0,7.0));
	this.addEnemyFormation(new KiteEnemyFormation(250.0,0.0,8.0));
	this.addEnemyFormation(new KiteEnemyFormation(150.0,0.0,9.0));
	this.addEnemyFormation(new KiteEnemyFormation(350.0,0.0,10.0));
};
$hxClasses["Stage1Scene"] = Stage1Scene;
Stage1Scene.__name__ = ["Stage1Scene"];
Stage1Scene.__super__ = GameScene;
Stage1Scene.prototype = $extend(GameScene.prototype,{
	update: function() {
...
var Stage2Scene = function(myShip,score,pressedFireButton,bullets) {
	...
	this.addEnemyFormation(new KiteEnemyFormation(150.0,0.0,2.0));
	this.addEnemyFormation(new KiteEnemyFormation(320.0,0.0,2.0));
	this.addEnemyFormation(new KiteEnemyFormation(250.0,0.0,4.0));
	this.addEnemyFormation(new KiteEnemyFormation(200.0,0.0,5.0));
	this.addEnemyFormation(new KiteEnemyFormation(150.0,0.0,6.0));
	this.addEnemyFormation(new KiteEnemyFormation(260.0,0.0,7.0));
	this.addEnemyFormation(new KiteEnemyFormation(120.0,0.0,8.0));
	this.addEnemyFormation(new KiteEnemyFormation(200.0,0.0,9.0));
	this.addEnemyFormation(new KiteEnemyFormation(350.0,0.0,10.0));
};
$hxClasses["Stage2Scene"] = Stage2Scene;
...
com.eclecticdesignstudio.control = {}
com.eclecticdesignstudio.control.KeyBinding = function() { }
$hxClasses["com.eclecticdesignstudio.control.KeyBinding"] = com.eclecticdesignstudio.control.KeyBinding;
com.eclecticdesignstudio.control.KeyBinding.__name__ = ["com","eclecticdesignstudio","control","KeyBinding"];
com.eclecticdesignstudio.control.KeyBinding.addOnPress = function(key,handler,repeat) {
	if(repeat == null) repeat = false;
	com.eclecticdesignstudio.control.KeyBinding.initialize();
	if(js.Boot.__instanceof(key,String)) key = com.eclecticdesignstudio.control.KeyBinding.getKeyCode(key);
	com.eclecticdesignstudio.control.KeyBinding.pressBindings.set(key,new com.eclecticdesignstudio.control.KeyBindingData(key,handler,repeat));
}
com.eclecticdesignstudio.control.KeyBinding.addOnRelease = function(key,handler) {
	com.eclecticdesignstudio.control.KeyBinding.initialize();
	if(js.Boot.__instanceof(key,String)) key = com.eclecticdesignstudio.control.KeyBinding.getKeyCode(key);
	com.eclecticdesignstudio.control.KeyBinding.releaseBindings.set(key,new com.eclecticdesignstudio.control.KeyBindingData(key,handler));
}
com.eclecticdesignstudio.control.KeyBinding.getKeyCode = function(key) {
	return HxOverrides.cca(key.toUpperCase(),0);
}
com.eclecticdesignstudio.control.KeyBinding.initialize = function() {
	if(!com.eclecticdesignstudio.control.KeyBinding.initialized) {
		nme.Lib.nmeGetCurrent().getStage().addEventListener(jeash.events.KeyboardEvent.KEY_DOWN,com.eclecticdesignstudio.control.KeyBinding.stage_onKeyDown);
		nme.Lib.nmeGetCurrent().getStage().addEventListener(jeash.events.KeyboardEvent.KEY_UP,com.eclecticdesignstudio.control.KeyBinding.stage_onKeyUp);
		com.eclecticdesignstudio.control.KeyBinding.initialized = true;
	}
}
com.eclecticdesignstudio.control.KeyBinding.stage_onKeyDown = function(event) {
	var key = event.keyCode;
	if(com.eclecticdesignstudio.control.KeyBinding.pressBindings.exists(key)) {
		var data = com.eclecticdesignstudio.control.KeyBinding.pressBindings.get(key);
		if(data.repeat || !com.eclecticdesignstudio.control.KeyBinding.keyPressed.exists(key)) data.handler.apply(data.handler,[]);
		com.eclecticdesignstudio.control.KeyBinding.keyPressed.set(key,true);
	}
}
com.eclecticdesignstudio.control.KeyBinding.stage_onKeyUp = function(event) {
	var key = event.keyCode;
	com.eclecticdesignstudio.control.KeyBinding.keyPressed.remove(key);
	if(com.eclecticdesignstudio.control.KeyBinding.releaseBindings.exists(key)) {
		var data = com.eclecticdesignstudio.control.KeyBinding.releaseBindings.get(key);
		data.handler.apply(data.handler,[]);
	}
}
com.eclecticdesignstudio.control.KeyBindingData = function(key,handler,repeat) {
	if(repeat == null) repeat = false;
	this.key = key;
	this.handler = handler;
	this.repeat = repeat;
};
$hxClasses["com.eclecticdesignstudio.control.KeyBindingData"] = com.eclecticdesignstudio.control.KeyBindingData;
com.eclecticdesignstudio.control.KeyBindingData.__name__ = ["com","eclecticdesignstudio","control","KeyBindingData"];
com.eclecticdesignstudio.control.KeyBindingData.prototype = {
	__class__: com.eclecticdesignstudio.control.KeyBindingData
}
...
com.eclecticdesignstudio.motion.actuators.SimpleActuator = function(target,duration,properties) {
	...
	if(com.eclecticdesignstudio.motion.actuators.SimpleActuator.shape == null) {
		...
		nme.Lib.nmeGetCurrent().getStage().addEventListener(jeash.events.Event.ENTER_FRAME,com.eclecticdesignstudio.motion.actuators.SimpleActuator.shape_onEnterFrame);
	}
};
...
com.eclecticdesignstudio.motion.actuators.SimpleActuator.prototype = $extend(com.eclecticdesignstudio.motion.actuators.GenericActuator.prototype,{
	...
	,move: function() {
		this.toggleVisible = Reflect.hasField(this.properties,"alpha") && js.Boot.__instanceof(this.target,jeash.display.DisplayObject);
...
com.eclecticdesignstudio.motion.easing.Expo.__properties__ = {get_easeOut:"getEaseOut"}
...
com.eclecticdesignstudio.motion.Actuate.stop = function(target,properties,complete,sendEvent) {
	...
	if(target != null) {
		if(js.Boot.__instanceof(target,com.eclecticdesignstudio.motion.actuators.GenericActuator)) (js.Boot.__cast(target , com.eclecticdesignstudio.motion.actuators.GenericActuator)).stop(null,complete,sendEvent); else {
...
com.eclecticdesignstudio.motion.actuators.FilterActuator = function(target,duration,properties) {
	this.filterIndex = -1;
	...
	if(js.Boot.__instanceof(properties.filter,Class)) {
		...
		var _g = 0, _g1 = (js.Boot.__cast(target , jeash.display.DisplayObject)).jeashGetFilters();
		while(_g < _g1.length) {
			var filter = _g1[_g];
			++_g;
		...
		this.filter = (js.Boot.__cast(target , jeash.display.DisplayObject)).jeashGetFilters()[this.filterIndex];
	}
};
...
com.eclecticdesignstudio.motion.actuators.FilterActuator.prototype = $extend(com.eclecticdesignstudio.motion.actuators.SimpleActuator.prototype,{
	update: function(currentTime) {
		...
		var filters = (js.Boot.__cast(this.target , jeash.display.DisplayObject)).jeashGetFilters();
...
com.eclecticdesignstudio.motion.actuators.MethodActuator = function(target,duration,properties) {
...
com.eclecticdesignstudio.motion.actuators.TransformActuator = function(target,duration,properties) {
	com.eclecticdesignstudio.motion.actuators.SimpleActuator.call(this,target,duration,properties);
};
...
com.eclecticdesignstudio.motion.actuators.TransformActuator.prototype = $extend(com.eclecticdesignstudio.motion.actuators.SimpleActuator.prototype,{
	...
	,initialize: function() {
		if(Reflect.hasField(this.properties,"colorValue") && js.Boot.__instanceof(this.target,jeash.display.DisplayObject)) this.initializeColor();
...
jeash.geom.Rectangle.prototype = {
	...
	,__properties__: {set_right:"set_right",get_right:"get_right",set_bottom:"set_bottom",get_bottom:"get_bottom",set_topLeft:"set_topLeft",get_topLeft:"get_topLeft"}
}
...
jeash.geom.Transform.prototype = {
	...
	,__properties__: {set_matrix:"setMatrix",get_matrix:"getMatrix",set_colorTransform:"setColorTransform"}
}
jeash.media = {}
...
jeash.net.URLLoader.prototype = $extend(jeash.events.EventDispatcher.prototype,{
	...
	,requestUrl: function(url,method,data,requestHeaders) {
		...
		} else throw "Cannot access path \"" + url + "\", access may be denied";
	}
...
jeash.text.TextField.prototype = $extend(jeash.display.InteractiveObject.prototype,{
	...
	,__properties__: $extend(jeash.display.InteractiveObject.prototype.__properties__,{set_text:"SetText",get_text:"GetText",set_defaultTextFormat:"jeashSetDefaultTextFormat",get_defaultTextFormat:"jeashGetDefaultTextFormat",set_autoSize:"SetAutoSize",set_borderColor:"SetBorderColor",set_background:"SetBackground",set_backgroundColor:"SetBackgroundColor",set_border:"SetBorder",set_wordWrap:"SetWordWrap",set_type:"SetType",get_type:"GetType"})
});
...
nme.Lib.__properties__ = {get_stage:"nmeGetStage",get_current:"nmeGetCurrent"}
...
nme.installer.Assets.getBitmapData = function(id,useCache) {
	...
	switch(id) {
	case "images/Bullet01.png":
		return (js.Boot.__cast(ApplicationMain.loaders.get("images/Bullet01.png").contentLoaderInfo.content , jeash.display.Bitmap)).bitmapData;
	case "images/Enemy01.png":
		return (js.Boot.__cast(ApplicationMain.loaders.get("images/Enemy01.png").contentLoaderInfo.content , jeash.display.Bitmap)).bitmapData;
	case "images/MyShip.png":
		return (js.Boot.__cast(ApplicationMain.loaders.get("images/MyShip.png").contentLoaderInfo.content , jeash.display.Bitmap)).bitmapData;
	}
	return null;
}
var $_;
